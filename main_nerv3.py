...
class NeRVLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = 'evaluation'):
        ...
        dist_locked = 10.0 * torch.ones(self.batch_size, device=_device)
        elev_locked = torch.zeros(self.batch_size, device=_device)
        azim_locked = torch.rand_like(elev_locked) * 2 - 1 # [0 1) to [-1 1)
        view_locked = make_cameras_dea(dist_locked, elev_locked, azim_locked)
        ...
        view_shape = [self.batch_size, 1] 
        # Roughly reconstruct the XR volume
        with torch.no_grad():
            estim3d = self.forward_volume(image2d=image2d, 
                                          elev=elev_hidden.view(view_shape), 
                                          azim=azim_hidden.view(view_shape),
                                          n_views=[1])
            estim3d = estim3d.sum(dim=1, keepdim=True)
        
        est_figure_ct_random = self.forward_screen(image3d=image3d, cameras=view_random, is_training=(stage=='train'))
        est_figure_ct_locked = self.forward_screen(image3d=image3d, cameras=view_locked, is_training=(stage=='train'))
         
        est_figure_xr_random = self.forward_screen(image3d=estim3d, cameras=view_random, is_training=(stage=='train'))
        est_figure_xr_hidden = self.forward_screen(image3d=estim3d, cameras=view_hidden, is_training=(stage=='train'))
        
        
        est_volume_ct_random, \
        est_volume_ct_locked, \
        est_volume_xr_random, \
        est_volume_xr_hidden = torch.split(
            self.forward_volume(
                ...
                elev=torch.cat([timezeros.view(view_shape), 
                                timezeros.view(view_shape), 
                                timezeros.view(view_shape),
                                timezeros.view(view_shape)]),
                azim=torch.cat([azim_random.view(view_shape), 
                                azim_locked.view(view_shape), 
                                azim_random.view(view_shape), 
                                azim_hidden.view(view_shape)]) * 90,
                n_views=[2, 2]
            ), self.batch_size
        )  
        
        # Reconstruct the appropriate XR
        rec_figure_ct_random = self.forward_screen(image3d=est_volume_ct_random, cameras=view_random, is_training=(stage=='train'))
        rec_figure_ct_locked = self.forward_screen(image3d=est_volume_ct_locked, cameras=view_locked, is_training=(stage=='train'))
         
        rec_figure_xr_random = self.forward_screen(image3d=est_volume_xr_random, cameras=view_random, is_training=(stage=='train'))
        rec_figure_xr_hidden = self.forward_screen(image3d=est_volume_xr_hidden, cameras=view_hidden, is_training=(stage=='train'))
        ...
        # Compute the loss
        im3d_loss_ct_random = self.l1loss(est_volume_ct_random, image3d) 
        im3d_loss_xr_hidden = self.l1loss(est_volume_xr_hidden, estim3d) 
        
        im2d_loss_ct_random = self.l1loss(rec_figure_ct_random, est_figure_ct_random)  
        im2d_loss_ct_locked = self.l1loss(rec_figure_ct_locked, est_figure_ct_locked)  
        im2d_loss_xr_random = self.l1loss(rec_figure_xr_random, est_figure_xr_random)  
        im2d_loss_xr_hidden = self.l1loss(rec_figure_xr_hidden, image2d)  
        
        im3d_loss = im3d_loss_ct_random + im3d_loss_xr_hidden
        im2d_loss = im2d_loss_ct_random + im2d_loss_ct_locked + im2d_loss_xr_random + im2d_loss_xr_hidden      
        ...
        if batch_idx==0:
            viz2d = torch.cat([
                ...
                torch.cat([image3d[..., self.vol_shape//2, :], 
                           ...
                           estim3d[..., self.vol_shape//2, :], ], dim=-2).transpose(2, 3),  
            ], dim=-2)
