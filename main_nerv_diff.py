...
class NeRVLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = 'evaluation'):
        ...
        class_labels = torch.cat([est_azim_random, est_azim_locked, est_azim_hidden]) * 1800
        ...
        if batch_idx==0:
            with torch.no_grad():
                ...
                for t in tqdm(self.noise_scheduler.timesteps):
                    ...
                    synth_images = self.noise_scheduler.step(model_output, 
                                                             t, 
                                                             synth_images, 
                                                             eta=0,
                                                             use_clipped_model_output=False,
                                                             generator=None).prev_sample
                gen_figure_ct_random, gen_figure_ct_locked, gen_figure_xr_hidden = torch.split(synth_images, [batchsz, batchsz, batchsz])
                gen2d = torch.cat([
